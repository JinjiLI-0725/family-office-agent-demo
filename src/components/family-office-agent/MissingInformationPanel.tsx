...
export function MissingInformationPanel({ items, lang, copy, embedded = false }: { items: MissingInformationItem[]; lang: Lang; copy: Record<string, string>; embedded?: boolean }) {
  const priorityLabel: Record<Priority, string> = { High: copy.high, Medium: copy.medium, Low: copy.low };
  return (
    <section className={embedded ? "" : "rounded-[2rem] border border-[#d8d1c4] bg-white p-6 shadow-[0_20px_70px_rgba(44,37,29,0.08)]"}>
