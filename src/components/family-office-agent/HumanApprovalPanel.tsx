...
export function HumanApprovalPanel({ selectedPromptId, lang, copy }: { selectedPromptId?: PromptId; lang: Lang; copy: Record<string, string> }) {
  const visibleItems = selectedPromptId ? approvalItems.filter((item) => item.promptIds.includes(selectedPromptId)) : approvalItems;
  return (
    <section id="professional-review" className="rounded-[2rem] border border-[#d8d1c4] bg-white p-6 shadow-[0_20px_70px_rgba(44,37,29,0.08)]">
